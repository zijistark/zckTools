...
/* zck::fp_decimal -- a class to represent parsed fixed-point decimal numbers accurately (i.e., fractional base-10^-N
 * component so that any parsed decimal number can be represented exactly).
 *
 * like with zck::date, our string input constructor assumes that the number-string is well-formed.
 *
...
std::ostream& operator<<(std::ostream& os, zck::fp_decimal<D> fp) {
    os << fp.integral();
